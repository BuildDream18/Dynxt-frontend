            ...
            <v-card class="d-flex flex-column justify-space-between  transparent"
              ...
              <v-card-text class="flex-1 d-flex">
                <v-container class="my-auto text-center-sm">
                  ...
                  <v-icon v-if="i==0" @click="copyAddress(i)" class="mt-6 icon-cls">mdi-content-copy</v-icon>
                  <div v-html="$md.render(slide.description)" v-bind:class="i==0 ? firstClass : normalClass" v-if="slide.description"></div>
                  <div class="3" v-if="slide.actionButtons.length>0">
...
<script>
    export default {
        ...
        data() {
            return {
                slide: 0,
                firstClass: 'grey--text mb-5 break-word txt-cursor',
